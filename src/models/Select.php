...
function getUtilisateur()
{
  global $pdo;
  try {
    $sql = "SELECT u.* , count(c.numero_commande) as nombre_commandes
     , COALESCE(SUM(l.quantite * p.prix), 0) AS total_depense
        FROM Utilisateur u
        left join commande c on c.id_client = u.id_utilisateur and c.statut = 'Livrer'
        left join ligne_commande l on l.numero_commande = c.numero_commande
